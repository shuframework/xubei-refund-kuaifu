...
public class StringUtil {
    ...
    public static String getSuffix(String str) {
        int index = str.lastIndexOf(SEPARATOR_POINT);
        if (index != -1) {
    ...
    public static String renameByAdd(String oldFileName, String addName, String type) {
        ...
        if (TYPE_PREFIX.equals(type)) {
            newName.append(addName).append(oldFileName);
        }
        if (TYPE_SUFFIX.equals(type)) {
            //截取后缀
            String suffix = "";
            String temp = oldFileName;
            int index = oldFileName.lastIndexOf(SEPARATOR_POINT);
            if (index != -1) {
                temp = oldFileName.substring(0, index);
                suffix = oldFileName.substring(index);
            }
            newName.append(temp).append(addName).append(suffix);
        }
...
//	}
    ...
    public static String capitalizeHump(String str) {
        ...
        String[] strArr = str.toLowerCase().split(SEPARATOR_UNDERLINE);
